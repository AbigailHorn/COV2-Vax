...
# Vaccination Scenarios: Realistic timeline
...
correlated.param.SCENARIOS.out <- correlated.param.SCENARIOS(fn_t_readin_input=fn_t_readin_input, ABC.out.mat=ABC.out.mat.scenarios, iter=iter, time.steps=time.steps, 
...
R0vsRate.Htotcum <- plot_heatmap(data=data,x.var=x.var,y.var=y.var, var.to.plot = var.to.plot, filter.coverage = filter.coverage, filter.65.delay = filter.65.delay, plot.title = plot.title, scale.x.labels=scale.x.labels,scale.y.labels=scale.y.labels)
#R0vsRate.D
...
plot.title <- "Peak Hospitalizations: Vax Rate vs. % Sero Negative Vax Comparison"
filter.coverage <- 0
filter.65.delay <- 0

R0vsRate.Htot <- plot_heatmap(data=data,x.var=x.var,y.var=y.var, var.to.plot = var.to.plot, filter.coverage = filter.coverage, filter.65.delay = filter.65.delay, plot.title = plot.title, scale.x.labels=scale.x.labels,scale.y.labels=scale.y.labels)
#R0vsRate.D
...
### Vaccination rate vs. % Sero+ Vaccinated: Total Deaths

```{r}
R0vsRate.D
```

### Vaccination rate vs. % Sero+ Vaccinated: Total Hospitalizations
...
# Vaccination Scenarios: 11-1-20 start date

This **counterfactual** vaccination scenario can be seen as modeling the effect size of the proposed vaccination policy of no sero+ individuals under a different epidemic context -- further from herd immunity at start of vaccination intervention.

Parameters modeled in scenarios:

- $R(t) = 1.9$ -- value estimated as of `r Sys.Date()` 
- **Vaccination starting date: 11-1-20**
- Vaccination rate 11-1-20 to 12-07-20: 8500 vaccines given per day
- Vaccination rate 12-07-21 -- on: 0, 10k, 20k, 30k, 40k, 50k vaccines given per day 
- Date 65+ group starts vaccination: 36 days after start (12-7-20)
- Vaccine coverage rate in 65+ (% final updake by population): 88% (from Understanding America Study)
- **Sero+ vaccination scenarios considered: 0%, 50%, 100% of recovered (sero+) individuals are (unnecessarily) vaccinated**
...
```{r scenarios-EARLY, include=FALSE}
...
time.steps <- last.date.plot.Vx - as.Date("2020-03-01") + 5
vx.delay <- 20
fn_t_readin_input = "fn_t_readin_EARLY.csv"
correlated.param.SCENARIOS.out <- correlated.param.SCENARIOS(fn_t_readin_input=fn_t_readin_input, ABC.out.mat=ABC.out.mat.scenarios, iter=iter, time.steps=time.steps, 
                                                             vx.scenarios.mat=vx.scenarios.mat, 
                                                             sero.scenario.in=sero.scenario.in,
                                                             mu.scenario.in=mu.scenario.in, 
