...
nvx_logic_check <- function(nvx_t,nvx_y,vx.delay,vx.coverage,delay.start.65.vx,start.date.vx){
  ...
  delay.start.65.vx <- 30
  #start.date.vx <- "2020-12-16"
  start.vx <- round(as.numeric(as.Date(start.date.vx) - as.Date("2020-03-01")) + start_time) + vx.delay - 1
  ...
  for (time in (1+start.vx + delay.start.65.vx):length(nvx_y.feas)){
    n.vx.when.65.start <- nvx_y.feas_cum[start.vx+delay.start.65.vx]
    if ((nvx_y.feas_cum[time] - n.vx.when.65.start) > vx.coverage*1e6 ) {   # 1e6: Population of 65+ in LA
      nvx_65_y.feas[time] <- 0
    }
  }
...
weighted.avg.scenario.65 <- function(X.mat, freq.PREV.q, freq.LAC.obs.age, logit.SEIR.est, psi.mat, percent.to.remove, factor.to.remove,
...
get.CI.SCENARIOS <- function(TEST.out) {
  ...
  traj.CI <- df.traj_dt[, list(
    ...
    by = c("date", "state.name","scenario.id","vx.id", "vx.coverage.id", "vx.start.date.id", "delay.start.65.vx.id","NPI.id")]
...
correlated.param.SCENARIOS <- function(ABC.out.mat, iter, time.steps, vx.scenarios.mat, NPI.scenarios.mat,
                                       X.mat=X.mat, freq.PREV.q=freq.PREV.q, freq.LAC.obs.age=freq.LAC.obs.age, logit.SEIR.est=logit.SEIR.est, psi.mat=psi.mat,
                                       vx.delay=vx.delay){
  ## Number of scenarios
  
  n.scenarios <- nrow(vx.scenarios.mat) * nrow(NPI.scenarios.mat)
  ...
  weighted.avg.scenarios.0 <- weighted.avg.scenario.65(X.mat=X.mat, freq.PREV.q=freq.PREV.q, freq.LAC.obs.age=freq.LAC.obs.age, logit.SEIR.est=logit.SEIR.est, psi.mat=psi.mat, percent.to.remove=percent.to.remove, factor.to.remove=factor.to.remove,
  ...
  for (NPI.idx in 1:nrow(NPI.scenarios.mat)){
    ...
    for (protect.idx in 1:nrow(vx.scenarios.mat)){
      ...
      nvx_ty_out <- nvx_logic_check(nvx_t=nvx_t,nvx_y=nvx_y,vx.delay=vx.delay,vx.coverage=vx.coverage, delay.start.65.vx = delay.start.65.vx, start.date.vx=start.date.vx) 
      ...
      start.time.effective.65 <- round(as.numeric(as.Date(start.date.vx) - as.Date("2020-03-01")) + start_time) + vx.delay + delay.start.65.vx - 1
      
      ## Get the % 65+ removed by day according to n.days.65.vx
      weighted.avg.scenarios <- weighted.avg.scenario.65(X.mat=X.mat, freq.PREV.q=freq.PREV.q, freq.LAC.obs.age=freq.LAC.obs.age, logit.SEIR.est=logit.SEIR.est, psi.mat=psi.mat, percent.to.remove=percent.to.remove, factor.to.remove=factor.to.remove,
                                                         n.days.65.vx=n.days.65.vx, vx.coverage.65=vx.coverage.65)
      
      #start.date.65.delay <- as.Date(start.date.65) + vx.delay
      #start.date.65.numeric.shift <- as.numeric(as.Date(start.date.65.delay)-Alpha_t_dates[1])   
      #start.time.effective.65
      ...
      for (idx in 1:nrow(ABC.out.mat)){
        ...
        x <- Vx_generator(Alpha_t=Alpha_t.scenario, Alpha_y=Alpha_y.scenario, Kappa_t=Alpha_t.scenario, Kappa_y=Kappa_y.scenario, Delta_t=Alpha_t.scenario, Delta_y=Delta_y.scenario, Beta_t=Beta_t, Beta_y=Beta_y, r_t=r_t, r_y=r_y, 
                          nvx_t=nvx_t.feas, nvx_y=nvx_y.feas, S_ini=1e7, E_ini=10, p_QV=p_V)
        ...
        ## BIND INCLUDING OFFSETING OBSERVED DATA BY START DATE
        TEST.out[[idx]] <- cbind(data.frame(scenario.id = paste0(scenario.idx, "_", name.vx, "_", NPI.name ) , vx.id = name.vx, vx.start.date.id=start.date.vx, vx.coverage.id = vx.coverage, delay.start.65.vx.id = delay.start.65.vx, NPI.id = NPI.name, R0.id = R0_scenario, par.id = idx, date = -start_time+TEST$step), TEST)
...
plot.SCENARIOS <- function(traj.CI, data.in, startDatePlot, endDatePlot, vars.to.plot, filter.scenarios) {
  ...
  ## PLOTTING
  traj.CI.line <- reshape2::melt(traj.CI[c("date","scenario.id",  "vx.id", "NPI.id", "state.name", "median")], id.vars = c("date", "scenario.id",  "vx.id", "NPI.id","state.name"))
  traj.CI.area <- reshape2::melt(traj.CI[c("date","scenario.id",  "vx.id", "NPI.id", "state.name", "low_95", "low_50", "up_50", "up_95")], id.vars = c("date", "scenario.id",  "vx.id", "NPI.id","state.name"))
