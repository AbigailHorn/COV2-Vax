...
plot_heatmap <- function(data, x.var, y.var, var.to.plot, filter.coverage, filter.65.delay, plot.title, scale.x.labels=NULL, scale.y.labels=NULL,rep.0=NULL,plot.0=NULL){
  ...
  if (!is.null(rep.0)){
    n.sero.vals <- length(unique(data$sero.id))
    data2 <- as.data.frame(lapply(data[1,],rep,n.sero.vals))
    data2[,"sero.id"] <- c(unique(data$sero.id))
    data3 <- rbind(data2, data[-1,])
    data = data3}
  
  if(is.null(plot.0)){
    data <- data %>% filter(vx.id!="0")
  }
